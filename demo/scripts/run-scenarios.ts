...
async function runScenarioOnce(scenario: Scenario): Promise<ScenarioResult> {
  ...
  if (scenario.script.length === 0) return { id: scenario.id, pass: true, ranAt, note: FIXTURE_ONLY_NOTE, steps: [] };
  if (scenario.requiresEnv && !process.env[scenario.requiresEnv]) {
    return { id: scenario.id, pass: true, ranAt, note: `skipped: start the dev server and the runner with ${scenario.requiresEnv}=1`, steps: [] };
  }
  ...
  return { id: scenario.id, pass: steps.every((step) => step.pass), ranAt, steps };
}
