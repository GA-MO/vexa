...
Status: **In progress** · Revision 3 · 2026-09-15 · phases 1–5 done, 6 on demand; supersedes the "test bench only" framing of `docs/demo-control-plan.md` without removing the runner
...
| 5 ✓ | `examples/embedded-widget` (2026-09-15): Vite + React, `vite/chat-api.ts` mounts `createVexaHandler` on the dev server with the scripted mock only, `vexa/mock` (`src/mock/scripted-model.ts`, first `examples/shared/mock-model.ts`) is the engine extracted from shop-admin (`createScriptedModel({ turns, prompts })`); four guide cards (theme + glow, labels/i18n, launcher position, overlay vs inline) each with a control, a prompt button and a snippet printed from the live config. Proved two library gaps: `styles.css` had no light defaults or `dark` variant for a non-shadcn host, and `VexaProvider`'s inline `{}` prop defaults looped the schema effect on every host re-render | runs with the mock, has 4 guides; console clean while switching every control |
| 5b ✓ | `examples/starter-next` and `examples/starter-vite` (2026-09-15): the Get started page as a folder, five files each (provider + overlay, chat route, `models.ts` with Claude behind `ANTHROPIC_API_KEY` and the `vexa/mock` scripted model as the fallback, a two-turn `mock.ts`, the stylesheet). The Vite one serves the route from `server.ts` on `Bun.serve` and proxies `/api` in dev (`dev.ts` starts both). The mock engine moved from `examples/shared/` into the library as `vexa/mock` so a starter depends on nothing but `vexa`. Not hosted on Pages (nothing to see beyond the widget); `/docs/examples/starters` and Get started link to the source | `bun run dev:starter-next` / `dev:starter-vite`, both build (`next build`, `vite build` + `bun server.ts`), typecheck covers both |
