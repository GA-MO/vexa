import type { LanguageModelV3CallOptions, LanguageModelV3Prompt, LanguageModelV3StreamPart, LanguageModelV3ToolResultOutput } from "@ai-sdk/provider";
import { MockLanguageModelV3, simulateReadableStream } from "ai/test";
import type { Spec, SpecPatch } from "vexa/protocol";
import { SCENARIOS } from "@/lib/scenarios";
import { allMockPrompts, matchesMockTurn, mockTurnSteps, mockTurnsOf } from "@/lib/scenarios/mock-prompts";
import type { MockContinuation, MockStep, MockToolStep, MockTurn } from "@/lib/scenarios/types";
...
function findTurn(text: string): MockTurn | undefined {
  return SCENARIOS.flatMap(mockTurnsOf).find((turn) => matchesMockTurn(turn, text));
}

function noScriptText(): string {
  const prompts = allMockPrompts(SCENARIOS).map((prompt) => `- ${prompt}`);
...
function chunksFor(options: LanguageModelV3CallOptions): LanguageModelV3StreamPart[] {
  const { text, index } = lastUserText(options.prompt);
  const turn = findTurn(text);
  if (!turn) return textOnlyStream(noScriptText());
...
export function createMockModel() {
  return new MockLanguageModelV3({
    ...
    doStream: async (options) => ({
      stream: simulateReadableStream({ chunks: chunksFor(options), chunkDelayInMs: CHUNK_DELAY_MS }),
    }),
  });
}
