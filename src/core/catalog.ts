...
/** Every component Vexa ships. A host extends this map through `extendCatalog`, never by editing it. */
export const vexaComponents = {
  Stack: {
    props: z.object({
  ...
  Card: {
    ...
    slots: ["default"],
    description:
      "Elevated surface for related content. Never nest Card in Card. `meta` is the scope line under the title (period, row count); `footnote` is the source line under a hairline. Keep `description` to one short sentence, or null when the body already says it.",
    example: {
      title: "Revenue by region",
      description: null,
      meta: "Sep 1 - Sep 22 - 6 regions",
      footnote: "Source: warehouse - certified - as of Sep 22",
    },
  },
  Grid: {
    props: z.object({
  ...
  Metric: {
    props: z.object({
      ...
      tone: z.enum(["good", "bad", "neutral"]).nullable(),
      delta: z.string().nullable(),
      note: z.string().nullable(),
      size: z.enum(["sm", "md", "lg"]).nullable(),
    }),
    description:
      "Single KPI. `delta` renders as a tinted pill; `trend` is the arrow direction and `tone` says whether that direction is good or bad — leave tone null when up is good, set it when it is not (overdue payments, churn, error rate). `detail` sits beside the value, `note` below. Use size lg for the one headline number of a card.",
    example: {
      label: "Users",
      value: "12,480",
      detail: "vs last week",
  ...
  Alert: {
    ...
    description: "Callout for tips, warnings, or errors. `meta` holds the numbers that back it (observed vs expected, gap).",
    example: {
      title: "Chiang Mai sell-out below plan",
      body: "Orders stopped on Sep 14 while stock on hand stayed flat.",
      tone: "danger",
      meta: "1,240 HL vs 1,980 expected - 37% gap",
    },
  },
  Separator: {
    props: z.object({}),
    ...
    props: z.object({
      columns: z.array(
        z.object({
          ...
          tone: z.enum(["default", "muted", "delta"]).nullable(),
        }),
      ),
      rows: z.array(z.record(z.string(), z.union([z.string(), z.number()]))),
    }),
    description:
      "Data table for more than three columns or more than eight rows. Number columns take align end; a column of signed percentages takes tone delta so it is colored by sign. For a ranked comparison of one number use RankList instead.",
    example: {
      columns: [
        { key: "name", label: "Name", align: null, tone: null },
        { key: "score", label: "Score", align: "end", tone: null },
        { key: "change", label: "vs last week", align: "end", tone: "delta" },
      ],
      rows: [{ name: "Alpha", score: 92, change: "-7.3%" }],
    },
  },
  RankList: {
    props: z.object({
      ...
      showRank: z.boolean().nullable(),
    }),
    description:
      "Ranked rows with a proportional bar: the fastest way to compare one number across up to ten named things. `share` is 0 to 1 of the largest row. Replaces a two-column Table and a horizontal BarChart.",
    example: {
      items: [
        { label: "Bangkok", value: "16.1M THB", share: 1, delta: "-7.3%", trend: "down", tone: null, note: null },
        { label: "South", value: "13.1M THB", share: 0.81, delta: "+2.1%", trend: "up", tone: null, note: null },
      ],
      showRank: true,
    },
  },
  ...
  LineChart: {
    ...
    description:
      "Line chart for trends over time. 1-4 series; handles 14-60 points and thins axis labels automatically. area=true fills under the line. A null value is a gap, never a zero: pad a forecast series with nulls over the actuals and give it style dashed.",
    example: {
      title: "Weekly active users",
      labels: ["W1", "W2", "W3", "W4", "W5"],
      series: [
        { name: "Users", values: [420, 480, 465, 530, null], style: null },
        { name: "Forecast", values: [null, null, null, 530, 560], style: "dashed" },
      ],
      area: true,
      showDots: true,
      format: "number",
...
export const catalog = defineCatalog(schema, { components: vexaComponents, actions: vexaActions });
...
/** Vexa's catalog plus the host's own components and actions, for `createVexaHandler({ catalog })` and `VexaProvider components`. */
export function extendCatalog(extension: CatalogExtension) {
  return defineCatalog(schema, {
    components: { ...vexaComponents, ...(extension.components ?? {}) },
    actions: { ...vexaActions, ...(extension.actions ?? {}) },
  } as never);
}
